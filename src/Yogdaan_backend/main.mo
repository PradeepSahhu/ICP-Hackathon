...
actor YogdaanDonationPlatform {
    ...
    private func calculateNGORank(ngoId: Nat): Nat {
        ...
        switch (ngos.get(ngoId)) {
            ...
            case (?ngo) {
                ...
                else if (totalScore > 800) { return 2 }
                else if (totalScore > 600) { return 3 }
                else if (totalScore > 400) { return 4 }
                else if (totalScore > 200) { return 5 }
                else { return 6 };
            };
        };
    };
    ...
    public shared({ caller }) func updateNGO(ngoId: Nat, name: Text, description: Text, location: Text, 
                                 completedProjects: Nat, beneficiaries: Nat, 
                                 transparencyScore: Nat, verified: Bool): async Result.Result<(), Error> {
    ...
    // Campaign Management
    public shared({ caller }) func createCampaign(ngoId: Nat, title: Text, description: Text, 
                                    purpose: Text, location: Text, targetAmount: Nat, 
                                    startDate: Int, endDate: Int): async Result.Result<Nat, Error> {
        if (not isAdmin(caller) and not isNgoOwner(ngoId, caller)) {
            return #err(#NotAuthorized);
        };
    ...
    public shared({ caller }) func updateCampaign(campaignId: Nat, title: Text, description: Text, 
                                    purpose: Text, targetAmount: Nat, 
                                    endDate: Int): async Result.Result<(), Error> {
    ...
    public query func getTopCampaigns(limit: Nat): async [Campaign] {
        ...
        let sortedByRaised = Array.sort(sortedCampaigns, func(a: Campaign, b: Campaign): {#less; #equal; #greater} {
            if (a.raisedAmount > b.raisedAmount) { #less }
            else if (a.raisedAmount < b.raisedAmount) { #greater }
            else { #equal }
        });
        
        // Take only the specified limit
        let actualLimit = if (sortedByRaised.size() < limit) { sortedByRaised.size() } else { limit };
    ...
    public query func getTopNGOs(limit: Nat): async [NGO] {
        ...
        let sortedByRank = Array.sort(sortedNGOs, func(a: NGO, b: NGO): {#less; #equal; #greater} {
        ...
        // Take only the specified limit
        let actualLimit = if (sortedByRank.size() < limit) { sortedByRank.size() } else { limit };
