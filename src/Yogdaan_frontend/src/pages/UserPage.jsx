...
const UserProfile = () => {
  ...
  const holdingsData = [
    { ngoId: 1, ngoName: "Green Earth Foundation", amount: 1200, campaignCount: 3 },
    { ngoId: 2, ngoName: "Water for All", amount: 850, campaignCount: 2 },
    { ngoId: 3, ngoName: "Child Education Fund", amount: 1500, campaignCount: 4 },
    { ngoId: 4, ngoName: "Disaster Relief Alliance", amount: 930, campaignCount: 2 },
    { ngoId: 5, ngoName: "Health Access Initiative", amount: 800, campaignCount: 1 },
  ];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-4">
            <Avatar className="h-20 w-20 border-2 border-white">
              ...
              <AvatarFallback className="bg-gray-800">{userData.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
            </Avatar>
            <div>
              <h1 className="text-2xl font-bold">{userData.name}</h1>
              <p className="text-gray-400">{userData.email}</p>
              <p className="text-gray-400 text-sm">Member since {formatDate(userData.joinedDate)}</p>
            </div>
          </div>
            ...
            <Card className="bg-gray-900 border-gray-800">
              <CardContent className="p-4 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">${userData.totalDonated.toLocaleString()}</p>
                </div>
              </CardContent>
            </Card>
        ...
        <Tabs defaultValue="holdings" className="w-full">
          <TabsList className="bg-gray-900 border-b border-gray-800 w-full justify-start mb-6">
            <TabsTrigger value="holdings" className="px-4 py-2">Holdings</TabsTrigger>
            <TabsTrigger value="watchlist" className="px-4 py-2">Watchlist</TabsTrigger>
            <TabsTrigger value="donations" className="px-4 py-2">Donation History</TabsTrigger>
              ...
              <CardContent>
                <div className="space-y-4">
                  {holdingsData.map((holding) => (
                    <div key={holding.ngoId} className="p-4 border border-gray-800 rounded-lg flex justify-between items-center">
                      <div>
                      ...
                      <div className="text-right">
                        <p className="text-lg font-bold">${holding.amount.toLocaleString()}</p>
                        <Button variant="link" className="px-0 text-blue-400 -mt-2">View Details</Button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <TabsContent value="watchlist" className="space-y-6">
            <Card className="bg-gray-900 border-gray-800">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button variant="outline" className="text-sm border-gray-700 hover:bg-gray-800">
                    ...
                    <div key={item.id} className="p-4 border border-gray-800 rounded-lg">
                      <div className="flex justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-400">{item.ngoName}</p>
                        </div>
                        <div className="text-right">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="text-left border-b border-gray-800">
                        ...
                        <th className="pb-3 font-medium text-gray-400 text-right">Date</th>
                      </tr>
                    </thead>
                    <tbody>
                      {donationHistory.map((donation) => (
                        <tr key={donation.id} className="border-b border-gray-800 hover:bg-gray-900/50">
                          <td className="py-4">
                            <Button variant="link" className="px-0 justify-start text-white hover:text-blue-400">
                              {donation.campaignTitle}
                            </Button>
                          </td>
                          <td className="py-4 text-gray-400">{donation.ngoName}</td>
                          <td className="py-4 text-right font-medium">${donation.amount}</td>
                          <td className="py-4 text-right text-gray-400">{formatDate(donation.date)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {pendingApprovals.length > 0 ? (
                  <div className="space-y-4">
                    {pendingApprovals.map((approval) => (
                      <div key={approval.id} className="p-4 border border-gray-800 rounded-lg">
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-sm text-gray-400">{approval.ngoName}</p>
                            <p className="text-sm text-gray-400 mt-1">Requested on {formatDate(approval.date)}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-lg font-bold">${approval.amount.toLocaleString()}</p>
                            <p className="text-sm text-gray-400">Current Approval: {approval.approvalRate}%</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-4">
                          <div className="w-full bg-gray-800 rounded-full h-2 mb-4">
                            <div 
                              className={`h-2 rounded-full ${approval.approvalRate >= 50 ? 'bg-green-500' : 'bg-yellow-500'}`}
                              style={{ width: `${approval.approvalRate}%` }}
                            ></div>
                          </div>
                          <div className="flex space-x-3">
                            <Button className="flex-1 bg-green-600 hover:bg-green-700">Approve</Button>
                            <Button variant="outline" className="flex-1 border-gray-700 hover:bg-gray-800">Decline</Button>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      You'll be notified when NGOs request to use funds you've donated.
                    </p>
                  </div>
                )}
              </CardContent>
